...
/// On-device benchmark for NVIDIA Parakeet-TDT-0.6b-v3 via sherpa-onnx. Mirror of
/// `BenchRunner` (the WhisperKit run) so the :benchmark scorer judges both identically;
/// only the runtime, model and provider differ. Writes raw-parakeet-ios.json.
///
/// Triggered by launching with `--earshot-bench-parakeet`; see `EarshotSampleApp`.
///
/// The ~640MB model is not bundled in git. Add the extracted
/// sherpa-onnx-nemo-parakeet-tdt-0.6b-v3-int8 directory (encoder.int8.onnx /
/// decoder.int8.onnx / joiner.int8.onnx / tokens.txt) to the app target as a folder
/// reference named `parakeet`, and add the sherpa-onnx SPM package + helper (see
/// `SherpaOnnxTranscriptionProvider`).
@MainActor
enum ParakeetBenchRunner {
    ...
    private static func execute() async throws {
        ...
        guard let modelDir = Bundle.main.url(forResource: "parakeet", withExtension: nil)?.path else {
            throw NSError(domain: "bench", code: 2, userInfo: [NSLocalizedDescriptionKey: "parakeet model dir not in bundle (see ParakeetBenchRunner doc)"])
        }

        // Timed cold load.
        let provider = SherpaOnnxTranscriptionProvider(modelDir: modelDir)
        let loadStart = Date()
        ...
        let dest = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("raw-parakeet-ios.json")
